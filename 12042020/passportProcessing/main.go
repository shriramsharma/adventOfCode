...
var validFields = map[string]func(value string)bool{
	"byr": birthYrValidation,
	"iyr": issueYrValidation,
	"eyr": expYrValidation,
	"hgt": hgtValidation,
	"hcl": hclValidation,"ecl":eclValidation,"pid":pidValidation,
}
...
func main() {
	...
	splitPassports := strings.Split(input, "\n\n")
	totalValidPassport := 0
	for _, p := range splitPassports{
		p = strings.ReplaceAll(p,"\n"," ")
		splitData := strings.Split(p, " ")
		m := make(map[string]string)
		for _, d := range splitData {
			splitFields := strings.Split(d, ":")
			f := strings.TrimSpace(splitFields[0])
			v := strings.TrimSpace(splitFields[1])
			if f == "" {
				continue
			}
			m[f] = v
		...
		totalValidPassport++
		for field, handler := range validFields {
			v, ok := m[field]
			if !ok {
				totalValidPassport--
				break
			...
			fmt.Printf("Field %s This is valid: %s \n",field,v)
		}
	}

	fmt.Println(totalValidPassport)

}

func yrValidation(yr string) bool {
	if len(yr) != 4 {
		return false
	}
	return true
...
func birthYrValidation(yr string)bool {
	if !yrValidation(yr) {
		return false
	}
	...
	if yrInt < 1920 || yrInt > 2002 {
		return false
	}
	return true
}
func issueYrValidation(yr string)bool {
	if !yrValidation(yr) {
		return false
	}
	yrInt, err := strconv.Atoi(yr)
	...
	if yrInt < 2010 || yrInt > 2020 {
		return false
	}
	return true
}
func expYrValidation(yr string)bool {
	if !yrValidation(yr) {
		return false
	}
	yrInt, err := strconv.Atoi(yr)
	...
	if yrInt < 2020 || yrInt > 2030 {
		return false
	}
	return true
}
func hgtValidation(height string) bool {
...
func hclValidation(color string) bool {
...
func pidValidation(pid string) bool {
...
func eclValidation(color string) bool {
	if _, ok := validEyeColors[color]; ok {
		return true
	}
	return false
}
...
var input=`byr:2010 pid:#1bb4d8 eyr:2021 hgt:186cm iyr:2020 ecl:grt
