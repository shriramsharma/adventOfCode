...
func main() {
	...
	splitNewLine := strings.Split(input, "\n")
	for _, s := range splitNewLine {
		rulePwdSplit := strings.Split(s,":") //"3-4 t:" "dttt" split this
		ruleSplit := strings.Split(rulePwdSplit[0]," ") //"3-4" "t" split this
		lowerUpperBoundSplit := strings.Split(ruleSplit[0], "-") //"3-4" split this
		upper, err := strconv.Atoi(lowerUpperBoundSplit[1])
		if err != nil {
			log.Fatal(err)
		}
		lower, err := strconv.Atoi(lowerUpperBoundSplit[0])
		if err != nil {
			log.Fatal(err)
		}
		searchChar := strings.TrimSpace(ruleSplit[1])
		searchOn := strings.TrimSpace(rulePwdSplit[1])

		if isValidSecondPasswordPolicy(searchOn,searchChar,lower,upper) {
			totalValidPasswords++
	...
	fmt.Printf("Total Valid Password: %d\n", totalValidPasswords)

}

func isValidSecondPasswordPolicy(searchOn string, searchChar string, firstPosition int, secondPosition int) bool {
	fmt.Printf("Password: %s  Character: %s  First Position: %d  Second Position: %d", searchOn, searchChar, firstPosition, secondPosition)

	doesContainInFirstPosition := false
	doesContainInSecondPosition := false
	if string(searchOn[firstPosition-1]) == searchChar {
		doesContainInFirstPosition = true
	}
	if string(searchOn[secondPosition-1]) == searchChar {
		doesContainInSecondPosition = true
	}

	if doesContainInFirstPosition && doesContainInSecondPosition {
		return false
	}
	if !doesContainInFirstPosition && !doesContainInSecondPosition {
		return false
...
func isValidFirstPasswordPolicy(searchOn string, searchChar string, lower int, upper int) bool {
	fmt.Printf("Password: %s  Character: %s  Lower: %d  Upper: %d", searchOn, searchChar, lower, upper)
	if len(searchOn) < lower {
		return false
	}

	if lower > 0 && !strings.Contains(searchOn,searchChar) {
		return false
	}
	count := 0
	for _, s := range searchOn {
		if 	string(s) == searchChar {
			count++
		}
	}
