...
func main() {
	...
	//canHoldMap := make(map[string]bool)
	rules := strings.Split(input, "\n")
	parents := make(map[string]*bag)
	for _, rule := range rules {
		splitOutsideBag := strings.Split(rule, "contain")
		if strings.TrimSpace(splitOutsideBag[1]) == "no other bags." {
			continue
		}
		splitInsideBag := strings.Split(splitOutsideBag[1], ",")
		children := make([]*bag, 0)
		for _, inBag := range splitInsideBag {
			inBag = strings.TrimSpace(inBag)
			qty, err := strconv.Atoi(string(inBag[0]))
			if err != nil {
				log.Fatal(err)
			}
			b := &bag{
				color:    removeTrailingSpacesAndBags(inBag[2:]),
				qty:      qty,
				children: nil,
			}
			children = append(children, b)
		}
		outsideBagColor := removeTrailingSpacesAndBags(splitOutsideBag[0])

		parent := &bag{
			color:    outsideBagColor,
			qty:      -1,
			children: children,
		}
		parents[outsideBagColor] = parent
	}

	//for k, v := range parents {
	//	//fmt.Println(v)
	//	visited := make(map[string]bool)
	//	canHoldMap[k] = travelBags(parents, v, visited, canHoldMap)
	//}
	//
	//total := 0
	//for k, v := range canHoldMap {
	//	if v {
	//		fmt.Println(k)
	//		total++
	//	}
	...
	visited := make(map[string]bool)
	fmt.Println(recurseShinyGoldBag(parents, parents["shiny gold"], visited))

}

func travelBags(parents map[string]*bag, bag *bag, visited map[string]bool, canHold map[string]bool) bool {

	if bag == nil {
		return false
	}

	if _, ok := canHold[bag.color]; ok {
		visited[bag.color] = true
		return canHold[bag.color]
	}

	if visited[bag.color] {
		return false
	}
	...
	for _, child := range bag.children {
		visited[bag.color] = true
		if strings.Contains(child.color, "shiny gold") {
			return true
		}
		canHold[child.color] = travelBags(parents, parents[child.color], visited, canHold)
		if !childCan {
			childCan = canHold[child.color]
		}
	}
...
func recurseShinyGoldBag(parents map[string]*bag, bag *bag, visited map[string]bool) int {
	...
	total := 0
	for _, child := range bag.children {
		total += child.qty + child.qty*recurseShinyGoldBag(parents, parents[child.color], visited)
	}
...
func removeTrailingSpacesAndBags(s string) string {
...
var input = `mirrored beige bags contain 1 drab brown bag, 3 dotted crimson bags.
